...
export const useWebSocket = (options: WebSocketOptions = {}) => {
  ...
  const connect = (): void => {
    ...
    try {
      ...
      socket.value.onclose = event => {
        ...
        emitEvent('disconnect' as WebSocketMessageType, {
          code: event.code,
          reason: event.reason,
        });
        ...
        if (
          wsOptions.autoReconnect &&
          reconnectAttempts.value < wsOptions.maxReconnectAttempts
        ) {
          scheduleReconnect();
        }
      };
  ...
  const unsubscribeFromPRDUpdates = (prdId: string): void => {
    send('prd_update' as WebSocketMessageType, {
      action: 'unsubscribe',
      prdId,
    });
  };
  ...
  const subscribeToAgentStatus = (agentId?: string): void => {
    send('agent_status' as WebSocketMessageType, {
      action: 'subscribe',
      agentId,
    });
  };
  ...
  const sendCollaborationMessage = (action: string, data: any): void => {
    send('collaboration' as WebSocketMessageType, {
      ...data,
      action,
    });
  };
  ...
  const joinCollaborationSession = (
    ...
    sendCollaborationMessage('join_session', {
  ...
  const leaveCollaborationSession = (sessionId: string): void => {
    sendCollaborationMessage('leave_session', {
      session_id: sessionId,
    });
  };
  
  const sendCollaborativeEdit = (
    sessionId: string,
    operation: any
  ): void => {
    sendCollaborationMessage('edit', {
      session_id: sessionId,
      operation,
    });
  };
  ...
  const updateCursorPosition = (
    ...
    sendCollaborationMessage('cursor_update', {
      session_id: sessionId,
  ...
  const requestSessionState = (sessionId: string): void => {
    sendCollaborationMessage('get_session_state', {
      session_id: sessionId,
    });
  };
