...
export const usePRDStore = defineStore('prd', {
  ...
  getters: {
    ...
    prdById: state => (id: string) => state.prds.find(prd => prd.id === id),

    currentPRDId: state => state.currentPRD?.id,

    averageHallucinationRate: state => {
      if (state.prds.length === 0) return 0;
      const sum = state.prds.reduce(
        (acc, prd) => acc + prd.hallucination_rate,
        0
      );
  ...
  actions: {
    ...
    async fetchPRD(projectId: string, prdId: string) {
      ...
      try {
        const { get } = useApiClient();
        const prd = await get<PRDResponse>(
    ...
    async generatePRD(projectId: string, request: PRDRequest) {
      ...
      try {
        ...
        const prd = await post<PRDResponse>(
          `/projects/${projectId}/prds/generate`,
          request
        );
    ...
    strategies: [
      {
        ...
        paths: ['prds', 'currentPRD'], // Only persist PRD data, not loading states
        // Exclude sensitive or temporary data from persistence
      },
    ],
  },
});
