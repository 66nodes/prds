...
async def lifespan(app: FastAPI):
    ...
    # Note: In production, workers should run in separate processes
    # For now, we'll start them in background tasks
    logger.info("Queue workers ready for background processing")
    ...
    await close_redis()
    logger.info("All database connections, LLM services, and Redis closed")
...
async def health_check():
    ...
    try:
        ...
        try:
            ...
            components_health["redis"] = redis_health
        except Exception as e:
            components_health["redis"] = {"status": "unhealthy", "error": str(e)}
        ...
        try:
            ...
            components_health["queue"] = {
                "status": "healthy" if active_workers > 0 else "degraded",
                "active_workers": active_workers,
                "total_workers": len(worker_stats)
            }
        except Exception as e:
            components_health["queue"] = {"status": "unhealthy", "error": str(e)}
