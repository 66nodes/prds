...
class Neo4jQueryOptimizer:
    ...
    def __init__(self):
        ...
        # Local query cache with TTL (fallback when Redis unavailable)
        self.query_cache = {}
    ...
    async def execute_optimized_query(
        ...
        if self._cache_service.is_available:
            cached_result = await self._cache_service.get_graph_data(
                query=query_name,
        ...
        try:
            ...
            async with self.driver.session() as session:
                ...
                if self._cache_service.is_available:
                    await self._cache_service.cache_graph_data(
                        query=query_name,
                        project_id=project_id,
                        data=data,
