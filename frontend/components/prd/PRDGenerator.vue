  ...
  <div class="max-w-4xl mx-auto p-6">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
        ...
        <!-- Validation Result -->
        <div v-if="validationResult" class="p-4 rounded-lg" :class="validationResultClass">
          <div class="flex">
            <div class="flex-shrink-0">
              <svg v-if="validationResult.hallucinationRate < 0.02" class="h-5 w-5 text-green-400" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="ml-3">
              <h3 class="text-sm font-medium">
                Validation {{ validationResult.hallucinationRate < 0.02 ? 'Passed' : 'Warning' }}
              </h3>
              <div class="mt-2 text-sm">
                <p>Hallucination Rate: {{ (validationResult.hallucinationRate * 100).toFixed(2) }}%</p>
                <p>Validation Score: {{ (validationResult.validationScore * 100).toFixed(1) }}%</p>
              </div>
            </div>
          </div>
        </div>
...
const { isValid: isFormValid, errors, validate, validateField, reset: resetValidation } = useFormValidation({
  title: {
  ...
  requirements: {
    ...
    rules: [
      validators.arrayMinLength(1, 'At least one requirement is needed')
    ]
  }
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const requestData: PRDRequest = {
      ...
      successMetrics: formData.value.successMetrics?.filter(m => m.trim() !== '')
    }
