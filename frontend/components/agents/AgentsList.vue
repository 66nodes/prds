  ...
  <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
    <div class="px-4 py-5 sm:p-6">
      ...
      <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="agent in agents" :key="agent.id" class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center">
              <div class="h-2 w-2 rounded-full mr-2"
                   :class="getStatusColor(agent.status)"></div>
              <span class="text-xs text-gray-500 dark:text-gray-400 capitalize">{{ agent.status }}</span>
            </div>
          </div>
          ...
          <div class="mt-4">
            ...
            <div class="mt-1 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1">
              <div class="bg-green-500 h-1 rounded-full" 
                   :style="{ width: `${agent.performance.successRate * 100}%` }"></div>
            </div>
          </div>
